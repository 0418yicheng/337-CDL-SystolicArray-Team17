    ...
    */
    ...
    always_comb begin : MathComb
        ...
        if (load_weight) begin
            ...
            logic in_is_nan, in_is_inf, in_is_zero;
            logic w_is_nan,  w_is_inf,  w_is_zero;
            logic sum_is_nan, sum_is_inf, sum_is_zero;

            n_out = in;

            in_is_nan  = (in[6:3] == 4'b1111) && (in[2:0] != 0);
            in_is_inf  = (in[6:3] == 4'b1111) && (in[2:0] == 0);
            in_is_zero = (in[6:0] == 0);

            w_is_nan  = (weight[6:3] == 4'b1111) && (weight[2:0] != 0);
            w_is_inf  = (weight[6:3] == 4'b1111) && (weight[2:0] == 0);
            w_is_zero = (weight[6:0] == 0);

            sum_is_nan  = (input_sum[6:3] == 4'b1111) && (input_sum[2:0] != 0);
            ...
            if(in_is_nan || w_is_nan || sum_is_nan) begin
                ...
                if(e_temp - 5'd7 >= 15) begin
                    inf = 1;
                    s_prod = weight[7] ^ in[7];
                    n_sum = {s_prod, 4'b1111, 3'b000};
                end
                else begin
                    if(e_temp < 5'd7)
                        e_prod = 4'd0;
                    ...
                    if(e_prod == 4'b1111) begin  //Check if shift pushed e_prod to inf
                        inf = 1;
                        n_sum = {s_prod, 4'b1111, 3'b000};
                    end
                    else begin
